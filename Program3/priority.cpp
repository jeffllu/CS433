...
int main() {
  ...
  while(getline(fin, str)) {
    ...
    processes.push_back(newProc);
		...
		//implementaion of Priority Scheduling
    for(int i = 0; i < processes.size(); i++) {
      int high = processes[i].priority;
      int pos = i;

			//checks current proccess and compares priority to next process
			//swaps processes if prio of the next is higher
  ...
  float turnaround = 0;
	...
	//outputs processes after priority scheduling 
	//displays current turnaround and waiting times
  for(int i = 0; i < processes.size(); i++) {
